...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		...
		while (count != 11)
		{
			...
			cout << "DataTime " << tm.wMonth << "." << tm.wDay << "." << tm.wYear << " " << endl
				<< tm.wHour + 3 << " Hours " << tm.wMinute << " Minuts " << tm.wSecond << " Seconds " << tm.wMilliseconds << " MiliSeconds " << endl
				<< inet_ntoa(client.sin_addr) << " Correction = " << setsincro.correction << ", Average Correction = " << average << endl;
			count++;
		}
