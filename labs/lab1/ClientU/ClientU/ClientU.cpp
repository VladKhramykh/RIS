	...
	try
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			cout << " DataTime " << tm.wMonth << "." << tm.wDay << "." << tm.wYear << endl
				<< " Hours " << tm.wHour + 3 << ":" << tm.wMinute << ":" << tm.wSecond << ":" << tm.wMilliseconds << " " << endl
				<< i + 1 << " " << getsincro.curvalue << " curvalue = " << setsincro.curvalue << " max correction/min correction: " << maxcor << "/" << mincor << endl << endl << endl;
